...
\frame{
    ...
    Marginal likelihood
    \begin{align}
    	P(x_1^m) &= \prod_{i=1}^m \sum_{z=1}^c P(X=x_i, Z=z) \\
			&= \prod_{i=1}^m \sum_{k=z}^c P(Z=z)P(X=x_i| Z=z)
	\end{align}
	}
...
\frame{
	...
	\begin{align}
		P(Z=z|X=x) &= \frac{P(Z=z, X=x)}{\sum_{k=1}^c P(Z=k, X=x)} \\
			&= \frac{P(Z=z)P(X=x|Z=z)}{\sum_{k=1}^c P(Z=k)P(X=x|Z=k)}
	\end{align}
...
\frame{
	...
	{\bf Expectation-Maximisation algorithm}
	
	~
	
	E-step: 
	\begin{itemize}
		\item complete data using the posterior $P(Z_1^m|x_1^m)$
	\end{itemize}
	...
	M-step: 
	\begin{itemize}
		\item climb the likelihood surface	
		\item for categorical distributions $P(O=o|C=c) = \theta_{c, o}$ \\
		~ $c$ and $o$ are categorical  \\
		~ $0 \le \theta_{c,o} \le 1$ ~ and ~ $\sum_{o \in \mathcal O} \theta_{c, o} = 1$
	\end{itemize}	
	
	\begin{align}
		\theta_{c,o} &= \frac{\mathbb E[n(c \rightarrow o | Z_1^m)]}{\sum_{o'} \mathbb E[n(c \rightarrow o' | Z_1^m)]}
	\end{align}
}
...
\frame{
    ...
    \plate {corpus} {(source-sentence) (e)} {$ S $};
    \end{tikzpicture}
    }
    \end{center}
    \end{column}
...
\frame{
	\frametitle{IBM1: marginal likelihood}
...
\begin{adjustwidth}{-1.5em}{-1em}
	...
	\begin{small}
	\begin{align}
		\mathbb E[n(\mathsf e \rightarrow \mathsf f | A_1^n)] &= \sum_{a_1=0}^m \dotsb \sum_{a_n=0}^m P(a_1^n|f_1^n,e_0^m) n(c \rightarrow d | A_1^n)  \\
