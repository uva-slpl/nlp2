...
\frame{
    ...
    \begin{column}{0.7\textwidth}
    	\begin{itemize}\pause
			...
			\begin{itemize}
				\item e.g. $f_\theta(c) = \softmax(g_\theta(c))$ \\ \pause
				and $g_\theta(c) = \tanh(W r(c) + b)$ \\ \pause
				and $r(c) = E c$ \pause				
			\end{itemize}
			\item with $\theta=(E, W,b)$
		\end{itemize}
    \end{column}
    \end{columns}
    \pause
...
\frame{
    ...
    \begin{column}{0.7\textwidth}
    	\begin{itemize}\pause
			...
			$z \sim \mathcal N(0, 1)$ \pause
			\item generate categorical observation $x$ from $z$ \\
			$x \sim P(X|Z=z)$  \pause
			\item where $P(X|Z=z) = \Cat(f_\theta(z))$  \pause
			\begin{itemize}
				\item e.g. $f_\theta(z) = \softmax(g_\theta(z))$ \\ \pause
				and $g_\theta(z) = \tanh(W z + b)$ \\
			\end{itemize}\pause
			\item with $\theta=(E, W,b)$ \pause
...
\frame{
    ...
    \begin{column}{0.7\textwidth}
    	\begin{itemize}
			...
			\item where 
			\begin{itemize}
				\item $\mu_\phi(x) = u_\phi(r_\phi(x))$ \\
				e.g. $r_\phi(x) = E_{\text{u}} x$ \\
				and $u_\phi(r) = \tanh(W_{\text{u}} r + b_{\text{u}})$ \pause
				\item $\sigma^2_\phi(x) = \exp(v_\phi(r'_\phi(x)))$ \\
				e.g. $r'_\phi(x) = E_{\text{v}} x$ \\
				and $v_\phi(r) = \tanh(W_{\text{v}} r + b_{\text{v}})$ \pause
			\end{itemize}
			\item with $\phi=(E_{\text{u}}, E_{\text{v}}, W_{\text{u}}, W_{\text{v}}, b_{\text{u}}, b_{\text{v}})$		\pause	
		\end{itemize}
    \end{column}
    \end{columns}
...
\frame{
	...
	For $Z \sim \mathcal N(\mu, \sigma^2)$ we can re-express $Z$ in terms of $E \sim \mathcal N(0,I)$
	\begin{itemize}
		\item $Z = \mu + \sigma E$
	\end{itemize} \pause
