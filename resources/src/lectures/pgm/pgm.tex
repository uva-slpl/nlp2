...
\frame[<+->]{
	\frametitle{Word alignment}
		...
		\item inference can be done exactly.
...
\frame[<+->]{
	\frametitle{Mixture models}
		\begin{itemize}
		\item IBM models are constrained mixture models.
		\item A mixture model consist of $k$ mixture components, each defines a distribution over the space $X$.
		\item Each component can specialise its distribution on a subset of the data.
		\item The probability of a mixture model with $k$ components assigns to $n$ data point is denoted by:
		
		\begin{equation}
			\begin{aligned}
				p(x_{1}^{n}) &=  \prod_{i=1}^{n} \sum_{j=1}^{k} p(x_i, y_i = j) \\
				&= \prod_{i=1}^{n} \sum_{j=1}^{k} p(y_i= j)p(x_i \mid y_i = j)
			\end{aligned}
		\end{equation}
