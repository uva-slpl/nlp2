...
def test():
    ...
    # let's get a source setence
    src_str = 'petit chien'
    src_fsa = make_fsa(src_str)
    print(src_fsa)

    # here I am going to use [D(x)] as the new start symbol
    ...
    # Let's constrain D(x) to sentences up to 3 words

    length_fsa = LengthConstraint(3, strict=False)
    print(length_fsa)
    target_forest = earley(projected_forest, length_fsa, Nonterminal("D(x)"), sprime_symbol=Nonterminal("D(x|n)"))
    print(target_forest)
    len(target_forest)

    # Let's check the paths in D(x|n)
    #paths = enumerate_strings(target_forest, Nonterminal('D(x|n)'))
    #print('Paths in D(x|n): %d' % len(paths))
    #for path in paths:
    #    print(path)
    
    # Let's check the paths in D(x, y)
    #paths = enumerate_strings(ref_forest, Nonterminal('D(x,y)'))
    #print('Paths in D(x,y): %d' % len(paths))
    #for path in paths:
    #    print(path)
